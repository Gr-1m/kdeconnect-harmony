...
- **对"实测证据"来源的更正（2026-09-26，重要）**：上文"App 侧 hilog 21:28:46 `net stack stopped` + 桌面同刻掉回 `(paired)`"
  这组证据，**很可能出自我自己的重装动作**而非用户按 Home —— `tools/sign-debug.sh` / `launch-app.sh restart`
  会 `force stop process` 再启动，`native stop` 因此出现、桌面随之短暂 `(paired)`（重启后重新连上）。
  标记为 [推断]（工具确会强停应用），故该组证据**不足以支撑"ohemu 上按 Home 即断链"**；
