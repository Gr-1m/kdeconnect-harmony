...
## sudo / root 政策（2026-09-26 用户指令）

- **任何需要 root 的动作一律交给用户执行**：udev 规则、`chmod` 设备节点、`sudoers`、`pkexec`、
  以 root 运行 `hdc`/`hdc` 服务、读写 root-only 路径等 —— agent **只把命令交给用户**，不代跑、不绕过。
- **agent 不得以任何方式获得持久的 root 级权限**（包括"为了调试方便"临时安装规则/提权）。
- 取证优先选**零 root 路径**：模拟器走 TCP(`hdc tconn 127.0.0.1:5555`)；真机优先走**无线调试**
  (`hdc tconn <手机IP>:<端口>`)；USB 路线确实需要一次性 udev 规则时，命令交由用户执行。
