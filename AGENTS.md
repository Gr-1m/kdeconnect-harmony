...
- **libxml2 soname 坑（2026-09-13 实测，挡住所有 Linux 侧构建）**：系统 libxml2 升到 2.15（`libxml2.so.16`），而 CLT 的 BiSheng `ld.lld` 依赖 `libxml2.so.2`，CMake 探测编译器即失败（`CMake will not be able to correctly generate this project`，与业务代码无关）。旁路不改系统：`mkdir -p /tmp/ohos_libshim && ln -sf /usr/lib/libxml2.so.16 /tmp/ohos_libshim/libxml2.so.2` 后 `LD_LIBRARY_PATH=/tmp/ohos_libshim hvigorw --no-daemon assembleHap`（`--no-daemon` 必需：已启动的 hvigor daemon 不会继承新环境变量）。
